...
export const ListFiles = () => {
    ...
    return (
        <div className='list-files border'>
            { files.map((file, i) => <ItemFile key={i} name={file} onClick={() => set_target_file(file)}/>) }
            <div className='item-file' onDoubleClick={onDoubleClickCreateFile}>
                <input className="new-file-input" type='text' onChange={onChangeInputNewFile} value={newFileName}></input>
            </div>
        </div>
    );
};